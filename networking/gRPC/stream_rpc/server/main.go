...
func (itemStore *ItemStore) ListStorageLayoutOfCenter(req *itemstore.StorageCenterLayoutRequest, stream itemstore.ItemStoreService_ListStorageLayoutOfCenterServer) error {
	itemStore.logger.Info("Handle ListStorageLayoutOfCenter")

	storageCenterId := req.StorageCenter
	availableSpace, ok := StorageCenterManager[storageCenterId.String()]
	if ok {
		for i, arr := range availableSpace {
			for j, val := range arr {
	...
	return status.Errorf(codes.InvalidArgument, "You need to provide a valid StorageCenter argument")
}
